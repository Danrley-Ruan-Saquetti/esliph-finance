export enum DatabaseCodesError {
    ...
    P1003 = 'Database {database_file_name} does not exist at {database_file_path}; Database {database_name}.{database_schema_name} does not exist on the database server at {database_host}:{database_port}; Database {database_name} does not exist on the database server at {database_host}:{database_port}.',
    ...
    P1012 = 'Expressions defined in the database schema are deprecated',
    ...
    P3007 = 'Some of the requested preview features are not yet allowed in schema engine. Please remove them from your data model before using migrations. (blocked: {list_of_blocked_features})',
    P3008 = 'The migration {migration_name} is already recorded as applied in the database.',
    P3009 = 'migrate found failed migrations in the target database, new migrations will not be applied. Read more about how to resolve migration issues in a production database: https://pris.ly/d/migrate-resolve {details}',
    ...
    P3014 = 'Prisma Migrate could not create the shadow database. Please make sure the database user has permission to create databases. Read more about the shadow database (and workarounds) at https://pris.ly/d/migrate-shadow. Original error: {error_code} {inner_error}',
    P3015 = 'Could not find the migration file at {migration_file_path}. Please delete the directory or restore the migration file.',
    P3016 = 'The fallback method for database resets failed, meaning Migrate could not clean up the database entirely. Original error: {error_code} {inner_error}',
    ...
    P3018 = 'A migration failed to apply. New migrations cannot be applied before the error is recovered from. Read more about how to resolve migration issues in a production database: https://pris.ly/d/migrate-resolve Migration name: {migration_name} Database error code: {database_error_code} Database error: {database_error}',
    ...
    P6001 = 'The URL is malformed; for instance, it does not use the prisma:// protocol.',
    P6002 = 'The API Key in the connection string is invalid.',
    P6003 = 'The included usage of the current plan has been exceeded. This can only occur on the free plan.',
    P6004 = 'The global timeout of Accelerate has been exceeded. You can find the limit here.',
    P6005 = 'The user supplied invalid parameters. Currently only relevant for transaction methods. For example, setting a timeout that is too high. You can find the limit here.',
    P6006 = 'The chosen Prisma version is not compatible with Accelerate. This may occur when a user uses an unstable development version that we occasionally prune.',
    P6008 = 'The engine failed to start. For example, it couldn\'t establish a connection to the database.',
    P6009 = 'The global response size limit of Accelerate has been exceeded. You can find the limit here.',
    P6010 = 'Your accelerate project is disabled. Please enable it again to use it.'
}
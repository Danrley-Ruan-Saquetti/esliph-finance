...
const schemaDTO = ValidatorService.schema.object({
    ...
    priority: ValidatorService.schema.coerce.number().nonnegative({ message: GLOBAL_FINANCIAL_TRANSACTION_DTO.priority.messageMustBePositive }).optional(),
    isObservable: ValidatorService.schema.boolean().optional(),
    isSendNotification: ValidatorService.schema.boolean().optional(),
    timesToRepeat: ValidatorService.schema.number().optional(),
    ...
    frequency: ValidatorService.schema
        ...
        .optional(),
    receiver: ValidatorService.schema.string().trim().optional(),
    sender: ValidatorService.schema.string().trim().optional(),
    expiresIn: ValidatorService.schema.coerce.date().transform(GLOBAL_DTO.date.transform).optional(),
    dateTimeCompetence: ValidatorService.schema.coerce.date().transform(GLOBAL_DTO.date.transform).optional(),
})
...
export class FinancialTransactionUpdateUseCase extends UseCase {
    ...
    private async update(data: FinancialTransactionModel.UpdateArgs, id: ID) {
        const updateResult = await this.transactionRepository.updateById(data, { id })

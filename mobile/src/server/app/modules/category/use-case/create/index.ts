...
export class CategoryCreateUseCase extends UseCase<CategoryCreateResponse, CategoryCreateArgs> {
    ...
    async perform(args: CategoryCreateArgs & CategoryCreateArgsHeader) {
        ...
        const responseAccount = await this.listenerClient.get('PU:accounts/find?id', { id: accountId })
        ...
        if (!order) {
            const allCategories = await this.listenerRepository.get('DB:categories/find-all', { accountId })
        ...
        const response = await this.listenerRepository.post('DB:categories/create', { accentColor, isFavorite, name, order: newOrder, accountId })
